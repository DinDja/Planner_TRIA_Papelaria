...
export function AddTransactionDialog({
  open, onClose, defaultType, defaultAccountId, editId,
}: {
  open: boolean; onClose: () => void; defaultType?: 'income' | 'expense'; defaultAccountId?: string; editId?: string
}) {
  const addTransaction = useFinanceStore((s) => s.addTransaction)
  const updateTransaction = useFinanceStore((s) => s.updateTransaction)
  const accounts = useFinanceStore((s) => s.accounts)
  const existing = useFinanceStore((s) => s.transactions.find((item) => item.id === editId))
  ...
  useEffect(() => {
    if (!open) return
    if (editId && existing) {
      setTitle(existing.title); setAmount(existing.amount); setType(existing.type); setCategory(existing.category)
      setDate(existing.date); setRecurrence(existing.recurrence); setPaymentMethod(existing.paymentMethod ?? '')
      setAccountId(existing.accountId ?? ''); setStatus(existing.status); setNotes(existing.notes ?? '')
    } else if (!editId) {
      reset()
    }
  ...
  const handleSave = () => {
    ...
    const data = {
      ...
      account: selectedAccount?.name ?? existing?.account,
      accountId: selectedAccount?.id ?? existing?.accountId,
      status,
    ...
    if (editId) {
      ...
      addTransaction(data)
      toast({ title: type === 'income' ? 'Receita adicionada!' : 'Despesa adicionada!', variant: 'success' })
    }
    reset(); onClose()
  }
  ...
  return (
    <Dialog open={open} onOpenChange={(o) => !o && onClose()}>
      <DialogContent title={editId ? 'Editar movimentação' : type === 'income' ? 'Nova receita' : 'Nova despesa'}>
        <div className="flex flex-col gap-4">
          ...
          <div className="flex justify-end gap-2 pt-1">
            ...
            <Button onClick={handleSave} className="rounded-xl shadow-md">{editId ? 'Salvar alterações' : 'Adicionar'}</Button>
          </div>
        </div>
      </DialogContent>
    </Dialog>
...
export function AddFixedBillDialog({ open, onClose, editId }: { open: boolean; onClose: () => void; editId?: string }) {
  const addFixedBill = useFinanceStore((s) => s.addFixedBill)
  const updateFixedBill = useFinanceStore((s) => s.updateFixedBill)
  const existing = useFinanceStore((s) => s.fixedBills.find((item) => item.id === editId))
  const [title, setTitle] = useState('')
  const [amount, setAmount] = useState(0)
  ...
  useEffect(() => {
    if (!open) return
    if (editId && existing) {
      setTitle(existing.title); setAmount(existing.amount); setCategory(existing.category)
      setPaymentMethod(existing.paymentMethod ?? ''); setDayOfMonth(existing.dayOfMonth)
    } else if (!editId) {
      setTitle(''); setAmount(0); setCategory('Moradia'); setPaymentMethod(''); setDayOfMonth(5)
    }
  }, [open, editId, existing])

  const handleSave = () => {
    if (!title.trim() || amount <= 0) { toast({ title: 'Preencha título e valor', variant: 'error' }); return }
    const data = { title: title.trim(), amount, category, paymentMethod: paymentMethod || undefined, dayOfMonth }
    if (editId) {
      updateFixedBill(editId, data)
      toast({ title: 'Conta fixa atualizada!', variant: 'success' })
    } else {
      addFixedBill(data)
      toast({ title: 'Conta fixa adicionada!', variant: 'success' })
    }
    onClose()
  }
  ...
  return (
    <Dialog open={open} onOpenChange={(o) => !o && onClose()}>
      <DialogContent title={editId ? 'Editar conta fixa' : 'Nova conta fixa'} description="Despesa que se repete todo mês.">
        <div className="flex flex-col gap-4">
          <div>
            <label className="text-sm font-medium mb-1.5 block">Título</label>
...
export function AddSubscriptionDialog({ open, onClose, editId }: { open: boolean; onClose: () => void; editId?: string }) {
  const addSubscription = useFinanceStore((s) => s.addSubscription)
  const updateSubscription = useFinanceStore((s) => s.updateSubscription)
  const existing = useFinanceStore((s) => s.subscriptions.find((item) => item.id === editId))
  const [name, setName] = useState('')
  const [amount, setAmount] = useState(0)
  ...
  useEffect(() => {
    if (!open) return
    if (editId && existing) {
      setName(existing.name); setAmount(existing.amount); setBillingCycle(existing.billingCycle); setCategory(existing.category)
    } else if (!editId) {
      setName(''); setAmount(0); setBillingCycle('monthly'); setCategory('')
    }
  }, [open, editId, existing])

  const handleSave = () => {
    if (!name.trim() || amount <= 0 || !category) { toast({ title: 'Preencha nome, valor e categoria', variant: 'error' }); return }
    const data = { name: name.trim(), amount, billingCycle, category }
    if (editId) {
      updateSubscription(editId, data)
      toast({ title: 'Assinatura atualizada!', variant: 'success' })
    } else {
      addSubscription(data)
      toast({ title: 'Assinatura adicionada!', variant: 'success' })
    }
    onClose()
  }
  ...
  return (
    <Dialog open={open} onOpenChange={(o) => !o && onClose()}>
      <DialogContent title={editId ? 'Editar assinatura' : 'Nova assinatura'} description="Serviço recorrente (Netflix, Spotify, etc).">
        <div className="flex flex-col gap-4">
          <div>
...
export function AddCardDialog({ open, onClose, editId }: { open: boolean; onClose: () => void; editId?: string }) {
  const addCard = useFinanceStore((s) => s.addCard)
  const updateCard = useFinanceStore((s) => s.updateCard)
  const existing = useFinanceStore((s) => s.cards.find((item) => item.id === editId))
  ...
  useEffect(() => {
    if (!open) return
    if (editId && existing) {
      setBank(existing.name); setBrand(existing.brand ?? CARD_BRANDS[0]); setLastDigits(existing.lastDigits ?? '')
      setClosingDay(existing.closingDay); setDueDay(existing.dueDay); setColor(existing.color)
    } else if (!editId) {
      setBank(''); setBrand(CARD_BRANDS[0]); setLastDigits(''); setClosingDay(1); setDueDay(10); setColor(COLORS[3])
    }
  }, [open, editId, existing])
  ...
  const handleSave = () => {
    ...
    if (editId) {
      updateCard(editId, { name: bank.trim(), brand, lastDigits: lastDigits || undefined, closingDay, dueDay, color })
  ...
  return (
    <Dialog open={open} onOpenChange={(o) => !o && onClose()}>
      <DialogContent title={editId ? 'Editar cartão de crédito' : 'Novo cartão de crédito'}>
        <div className="flex flex-col gap-4">
          <div>
...
export function AddInstallmentDialog({ open, onClose, editId }: { open: boolean; onClose: () => void; editId?: string }) {
  ...
  const existing = useFinanceStore((s) => s.installments.find((item) => item.id === editId))
  const [title, setTitle] = useState('')
  ...
  const handleSave = () => {
    ...
    if (editId) {
      updateInstallment(editId, { ...data, currentInstallment: existing?.currentInstallment ?? 1 })
      toast({ title: 'Parcelamento atualizado!', variant: 'success' })
    } else {
      addInstallment(data)
      toast({ title: 'Parcelamento adicionado!', variant: 'success' })
    }
    onClose()
  }
  ...
  return (
    <Dialog open={open} onOpenChange={(o) => !o && onClose()}>
      <DialogContent title={editId ? 'Editar parcelamento' : 'Novo parcelamento'}>
        <div className="flex flex-col gap-4">
          <div>
...
export function GoalDialog({ open, onClose, editId }: { open: boolean; onClose: () => void; editId?: string }) {
  ...
  useEffect(() => {
    ...
    setCurrentAmount(existing?.currentAmount ?? 0)
    setDeadline(existing?.deadline ?? '')
    setColor(existing?.color ?? COLORS[2])
  }, [open, existing])
